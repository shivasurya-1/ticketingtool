    ...
    const fetchData = async () => {
      try {
        ...
        if (
          ...
          if (projectDetailsResponse.data[0].data) {
            setProjectData(projectDetailsResponse.data[0].data);
          }

          // Set project list for dropdown
          const allProjects =
            projectDetailsResponse.data[0].data?.map((p) => p.project_name) ||
            [];
          ...
          // Set projects per requestor mapping
          if (
            projectDetailsResponse.data[1] &&
            projectDetailsResponse.data[1].projects
          ) {
            const projectMapping = {};
            projectDetailsResponse.data[1].projects.forEach((item) => {
              projectMapping[item.name] = item.projects;
            });
