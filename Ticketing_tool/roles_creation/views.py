...
class RolePermissionAPIView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            permissions = serializer.validated_data['permission']  # already resolved
    ...
    #     permission_checker = HasRolePermission()

    #     try:
    #         logger.info(f"Request Method: {request.method}")
    #         logger.info(f"Request Data: {request.data}")

    #         if not permission_checker.has_permission(request, permission_required):
    ...
    #                 role=request.data.get('role'), permission=request.data.get('permission')
    #             ).exists():
    ...
    #     except Exception as e:
    #         logger.error(f"Unexpected error. Request: {request}, Error: {str(e)}")
    #         return Response({"error": f"An unexpected error occurred: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    def put(self, request, pk):
