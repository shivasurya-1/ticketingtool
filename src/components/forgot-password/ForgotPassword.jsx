...
const ForgotPassword = () => {
  ...
  const { otpVerified, loading, error } = useSelector(
  ...
  const handleOtpSubmit = async () => {
    try {
      dispatch(verifyOTP());
      const response = await axiosInstance.post(verifyotp_url, {
        otp,
        email,
      });
  ...
  const handlePasswordReset = async () => {
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Issue in Changing the Password";
